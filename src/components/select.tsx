...
const Select: FC<SelectPropsType> = ({
    ...
    return (
        <SelectContext.Provider value={{ isOpen, setIsOpen, fallbackValue, setFallbackValue, onValueChange }}>
            <div ref={ref} className={cn(base, className ?? "")}>{children}</div>
...
const SelectList: FC<SelectListPropsType> = ({ children, className }) => {
    ...
    return (
        <div
            className={cn(baseL, className ?? "", isOpen ? "scale-y-100" : "")}
        >
            {children}
        </div>
    );
...
const SelectItem: FC<SelectItemPropsType> = ({
    ...
    const { fallbackValue, setIsOpen, setFallbackValue, onValueChange } = useSelect();
    ...
    return (
        <Button
            type="button"
            variant={
                children && children.toString() === fallbackValue ? "secondary" : "ghost"
            }
            size="small"
